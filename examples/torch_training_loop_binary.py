"""
Example: PyTorch custom training loop — binary classification.

Run:
    pip install "experiment-saver[torch]"
    python examples/torch_training_loop_binary.py
"""
...
def build_model(input_dim: int = 16) -> nn.Module:
...
def evaluate(model, loader, criterion, device):
    ...
    for X, y in loader:
        ...
        correct += ((out >= 0.5).long() == y.long()).sum().item()
...
def main():
    ...
    cfg = TorchExperimentConfig(
        run_dir="runs/torch_binary_001",
        monitor="val_loss",
        patience=5,
        verbose=1,
        model_input_size=(1, 16),           # for torchinfo summary (optional)
        bundle_artifacts=False,
    )

    saver = TorchExperimentSaver(cfg, class_names=["negative", "positive"])
    ...
    saved_paths = saver.save_after_fit(
        ...
        last_epoch=epoch,
        device=device,
        extra_config={"learning_rate": 1e-3, "architecture": "dense_3layer"},
    )
